...
/** POST Form 데이터를 Context parmas에 저장 **/
...
func staticHandler(next HandlerFunc) HandlerFunc {
	...
	return func(c *Context) {
		// http 메서드가 GET이지만, HEAD가 아니면 바로 다음 핸들러 수행
		...
		// url 경로가 디렉터리면 indexFile 사용
		if fi.IsDir() {
			// 디렉터리 경로를 url로 사용하면 경로 끝에 "/"를 붙임
		...
		// file 내용 전달(next 핸들러로 제어권을 넘기지 않고 요청 처리 종료)
